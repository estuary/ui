export type catalog =
    /* Catalog Each catalog source defines a portion of a Flow Catalog, by defining collections, derivations, tests, and materializations of the Catalog. Catalog sources may reference and import other sources, in order to collections and other entities that source defines. */ {
        $schema?: /* JSON-Schema against which the Catalog is validated. */
        string | null;
...
export type BucketType =
    /* BucketType is a provider of object storage buckets, which are used to durably storage journal fragments. */
    'AZURE' | 'GCS' | 'S3';

export type Capture =
    /* Capture names are paths of Unicode letters, numbers, '-', '_', or '.'. Each path component is separated by a slash '/', and a name may not begin or end in a '/'. */ string;
...
export type Collection =
    /* Collection names are paths of Unicode letters, numbers, '-', '_', or '.'. Each path component is separated by a slash '/', and a name may not begin or end in a '/'. */ string;
...
export type CompositeKey =
    /* Ordered JSON-Pointers which define how a composite key may be extracted from a collection document. */ JsonPointer[];
...
export type CompressionCodec =
    ...
    'GZIP' | 'GZIP_OFFLOAD_DECOMPRESSION' | 'NONE' | 'SNAPPY' | 'ZSTANDARD';

export type Config =
    /* A configuration which is either defined inline, or is a relative or absolute URI to a configuration file. */
    | {
          [k: string]: unknown;
      }
    | string;
...
export type Derivation =
    /* A derivation specifies how a collection is derived from other collections. A collection without a derivation is a "captured" collection, into which documents are directly ingested. */ {
        register?: /* Register configuration of this derivation. */ Register;
        shards?: /* Template for shards of this derivation task. */ ShardTemplate;
        transform: /* Transforms which make up this derivation. */ {
            [k: string]: TransformDef;
...
export type FragmentTemplate =
    /* A FragmentTemplate configures how journal fragment files are produced as part of a collection. */ {
        compressionCodec?: /* Codec used to compress Journal Fragments. */
        string | null;
        flushInterval?: /* Maximum flush delay before in-progress fragments are closed and persisted into cloud storage. Intervals are converted into uniform time segments: 24h will "roll" all fragments at midnight UTC every day, 1h at the top of every hour, 15m a :00, :15, :30, :45 past the hour, and so on. If not set, then fragments are not flushed on time-based intervals. */
        string | null;
        length?: /* Desired content length of each fragment, in megabytes before compression. When a collection journal fragment reaches this threshold, it will be closed off and pushed to cloud storage. If not set, a default of 512MB is used. */
        number | null;
        retention?: /* Duration for which historical fragments of a collection should be kept. If not set, then fragments are retained indefinitely. */
        string | null;
    };
...
export type IngestConfig = /* Ingest source specification. */ Record<
    string,
    unknown
>;
...
export type JsonPointer =
    /* JSON Pointer which identifies a location in a document. */ string;

export type Lambda =
    /* Lambdas are user functions which are invoked by the Flow runtime to process and transform source collection documents into derived collections. Flow supports multiple lambda run-times, with a current focus on TypeScript and remote HTTP APIs.
...
export type Materialization =
    /* Materialization names are paths of Unicode letters, numbers, '-', '_', or '.'. Each path component is separated by a slash '/', and a name may not begin or end in a '/'. */ string;
...
export type MaterializationDef =
...
export type MaterializationEndpoint =
...
export type MaterializationFields =
...
export type Prefix =
    /* Prefixes are paths of Unicode letters, numbers, '-', '_', or '.'. Each path component is separated by a slash '/'. Prefixes may not begin in a '/', but must end in one. */ string;
...
export type Register =
    /* Registers are the internal states of a derivation, which can be read and updated by all of its transformations. They're an important building block for joins, aggregations, and other complex stateful workflows.
...
export type RelativeUrl =
    /* A URL identifying a resource, which may be a relative local path with respect to the current resource (i.e, ../path/to/flow.yaml), or may be an external absolute URL (i.e., http://example/flow.yaml). */ string;
...
export type ShardTemplate =
    /* A ShardTemplate configures how shards process a catalog task. */ {
        disable?: /* Disable processing of the task's shards. */ boolean;
        hotStandbys?: /* Number of hot standbys to keep for each task shard. Hot standbys of a shard actively replicate the shard's state to another machine, and are able to be quickly promoted to take over processing for the shard should its current primary fail. If not set, then no hot standbys are maintained. EXPERIMENTAL: this field MAY be removed. */
        number | null;
        logLevel?: /* Log level of this tasks's shards. Log levels may currently be "error", "warn", "info", "debug", or "trace". If not set, the effective log level is "info". */
        string | null;
        ...
        string | null;
        readChannelSize?: /* Size of the reader channel used for decoded documents. Larger values are recommended for tasks having more than one shard split and long, bursty transaction durations. If not set, a reasonable default (currently 65,536) is used. EXPERIMENTAL: this field is LIKELY to be removed. */
        number | null;
...
export type Test =
    /* Test names are paths of Unicode letters, numbers, '-', '_', or '.'. Each path component is separated by a slash '/', and a name may not begin or end in a '/'. */ string;

export type TestDocuments =
...
export type Transform =
    /* Transform names are Unicode letters, numbers, '-', '_', or '.'. */ string;
