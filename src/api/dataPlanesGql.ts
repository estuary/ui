...
// Cloud provider types - API uses uppercase, consumers use lowercase
type CloudProviderApi = 'AWS' | 'GCP' | 'AZURE' | 'LOCAL';
export type CloudProvider = 'aws' | 'gcp' | 'azure' | 'local';

const toCloudProvider = (api: CloudProviderApi): CloudProvider =>
    api.toLowerCase() as CloudProvider;
...
const toDataPlaneNode = (node: DataPlaneGqlNode): DataPlaneNode => {
    const cloudProvider = toCloudProvider(node.cloudProvider);
    return {
        ...node,
        cloudProvider,
