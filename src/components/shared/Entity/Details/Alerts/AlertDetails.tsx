...
function AlertDetailsWrapper({
    datum,
    hideLabel,
    short,
}: AlertDetailsWrapperProps) {
    ...
    return (
        <Stack>
            {hideLabel ? null : <Typography>{detail.label}</Typography>}
            <ServerError datum={datum} detail={detail} short={short} />
        </Stack>
    );
}
