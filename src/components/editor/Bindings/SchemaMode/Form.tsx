...
import useSupportedOptions from 'hooks/SchemaMode/useSupportedOptions';
import useUpdateSchemaMode from 'hooks/SchemaMode/useUpdateSchemaMode';
...
function SchemaModeForm({ bindingIndex = -1 }: Props) {
    const intl = useIntl();
    ...
    const { updateSchemaMode } = useUpdateSchemaMode();
    ...
    const updateServer = useCallback(
        async (value?: AutoCompleteOption | null) => {
            ...
            updateSchemaMode(value?.val, bindingMetadata)
                .then(() => {
        ...
        [bindingMetadata, enqueueSnackbar, intl, setFormState, updateSchemaMode]
    );
                    ...
                    return (
                        <TextField
                            ...
                            helperText={
                                invalidSetting
                                    ? intl.formatMessage({
                                          id: 'schemaMode.error.message',
                                      })
                                    : undefined
                            }
                            label={intl.formatMessage({
                                id: 'schemaMode.input.label',
                            })}
                            variant="standard"
                        />
                    );
