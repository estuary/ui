...
const stateSelectors: StoreSelector<CreationState> = {
    formData: (state) => state.endpointConfig.data,
    setConfig: (state) => state.setEndpointConfig,
    showValidation: (state) => state.formState.showValidation,
    status: (state) => state.formState.status,
};

function NewMaterializationEndpointConfigForm({ endpointSchema }: Props) {
    ...
    }, [endpointSchema, setConfig, setDeReffedSchema]);
...
export default NewMaterializationEndpointConfigForm;
