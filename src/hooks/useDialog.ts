...
type OpenArgs<K extends DialogKey> =
    ContextParams<K> extends never
        ? []
 ...
 */
function openDialogParams<K extends DialogKey>(key: K, ...args: OpenArgs<K>) {
...
/**
 * Returns a URL search string (e.g. `?dialog=...&prefix=...`) that opens the
 * given dialog while preserving existing search params.
 */
export function useDialogLink<K extends DialogKey>(
    key: K,
    ...args: OpenArgs<K>
) {
